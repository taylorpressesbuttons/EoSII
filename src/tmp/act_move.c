...
/* void move_char( CHAR_DATA *ch, int door ) */
{
    ...
    if (!IS_NPC(ch))
    {
        ...
        {
            if (iClass != prime_class(ch) && to_room->vnum == class_table[iClass].guild)
            {
        ...
        {
            if ((!IS_AFFECTED(ch, AFF_FLYING)) && (ch->race != RACE_PIXIE) && (ch->race != RACE_ELDER))
            {
        ...
        WAIT_STATE(ch, 1);
        if ((!IS_AFFECTED(ch, AFF_FLYING)) && (ch->race != RACE_PIXIE)
            && (ch->race != RACE_LIZARDMAN) && (ch->race != RACE_ELDER))
            ch->move -= move;
    }
    ...
    if (IS_SET(to_room->room_flags, ROOM_NOFLOOR) && !IS_AFFECTED(ch, AFF_FLYING)
        && ((pexit = to_room->exit[5]) != NULL) && ((to_room = pexit->to_room) != NULL)
...
int find_door(CHAR_DATA *ch, char *arg, bool pMsg)
{
    ...
    else
    {
        ...
        {
            if ((pexit = ch->in_room->exit[door]) && IS_SET(pexit->exit_info, EX_ISDOOR)
                && pexit->keyword && is_name(ch, arg, pexit->keyword))
                return door;
        }
        if (pMsg)
...
void do_hide(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_hide))
    {
        send_to_char(C_DEFAULT, "Huh?\n\r", ch);
        return;
    }
...
void do_train(CHAR_DATA *ch, char *argument)
{
    ...
    else if (!is_class(ch, CLASS_VAMPIRE) && !str_cmp(arg, "mana"))
    {
        cost = 1;
        bone_flag = 1;
    ...
    else if (is_class(ch, CLASS_VAMPIRE) && !str_cmp(arg, "bp"))
    {
        cost = 3;
        bone_flag = 1;
        pAbility = &ch->perm_bp;
...
void do_chameleon(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_chameleon))
    {
        send_to_char(C_DEFAULT, "Huh?\n\r", ch);
        return;
    }
...
void do_heighten(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_heighten))
    {
        send_to_char(C_DEFAULT, "Huh?\n\r", ch);
        return;
    }
...
void do_shadow(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_shadow))
    {
        send_to_char(C_DEFAULT, "Huh?\n\r", ch);
        return;
    }
...
void do_bash(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_bash_door))
    {
...
void do_push(CHAR_DATA *ch, char *argument)
{
    ...
    if ((victim->level >= LEVEL_IMMORTAL)
        || (IS_NPC(victim)
            && ((victim->pIndexData->pShop) || IS_SET(ch->in_room->room_flags, ROOM_SMITHY)
                || IS_SET(ch->in_room->room_flags, ROOM_BANK) || IS_SET(victim->act, ACT_NOPUSH))))
    {
...
void do_drag(CHAR_DATA *ch, char *argument)
{
    ...
    if ((victim->level >= LEVEL_IMMORTAL)
        || (IS_NPC(victim)
            && ((victim->pIndexData->pShop) || IS_SET(ch->in_room->room_flags, ROOM_SMITHY)
                || IS_SET(ch->in_room->room_flags, ROOM_BANK) || IS_SET(victim->act, ACT_NODRAG))))
    {
        act(AT_BLUE, "$N ignores you.", ch, NULL, victim, TO_CHAR);
        return;
    }
...
void check_nofloor(CHAR_DATA *ch)
{
    ...
    if (IS_SET(ch->in_room->room_flags, ROOM_NOFLOOR) && ((pexit = ch->in_room->exit[5]) != NULL)
        && ((to_room = pexit->to_room) != NULL) && (ch->race != RACE_PIXIE)
        && (ch->race != RACE_ELDER))
...
void do_shadow_walk(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_shadow_walk))
    {
...
*/
    ...
    {
        if (victim->deleted || victim->in_room->area != ch->in_room->area
            || IS_SET(victim->in_room->room_flags, ROOM_PRIVATE)
            || IS_SET(victim->in_room->room_flags, ROOM_SOLITARY)
            || IS_SET(ch->in_room->room_flags, ROOM_NO_SHADOW)
            || IS_SET(victim->in_room->room_flags, ROOM_NO_SHADOW)
            || IS_SET(victim->act, ACT_NOSHADOW) || IS_AFFECTED(victim, AFF_NOASTRAL)
...
void do_scent(CHAR_DATA *ch, char *argument)
{
    ...
    for (dir = 0; dir <= 5; dir++)
    {
        ...
        for (dis = 0; dis <= 1; dis++)
        {
            ...
            if (!(pexit = next_room->exit[dir]) || !(next_room = pexit->to_room))
                break;
        }
    }
