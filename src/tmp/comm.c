...
void nanny(DESCRIPTOR_DATA *d, char *argument)
{
    ...
    switch (d->connected)
    {
        ...
        case CON_GET_NEW_CLASS:
            ...
            for (iClass = 0; iClass < MAX_CLASS; iClass++)
            {
                if (!str_prefix(argument, class_table[iClass].who_long) && class_table[iClass].races[ch->race])
                {
                    ch->class[0] = iClass;
                    break;
                }
        ...
        case CON_WANT_MULTI:
            switch (*argument)
            {
                ...
                case 'Y':
                    ...
                    for (iClass = 0; iClass < MAX_CLASS; iClass++)
                    {
                        if (class_table[iClass].races[ch->race] && prime_class(ch) != iClass
        ...
        case CON_GET_2ND_CLASS:
            ...
            for (iClass = 0; iClass < MAX_CLASS; iClass++)
            {
                if (!str_prefix(argument, class_table[iClass].who_long)
                    && class_table[iClass].races[ch->race] && prime_class(ch) != iClass
                    && class_table[prime_class(ch)].multi[iClass])
                {
                    ch->class[1] = iClass;
                    break;
...
*/
        ...
        case CON_WANT_MULTI_2:
            switch (*argument)
            {
                ...
                case 'Y':
                    ...
                    {
                        if (class_table[iClass].races[ch->race] && prime_class(ch) != iClass
                            && ch->class[1] != iClass && class_table[prime_class(ch)].multi[iClass]
            ...
            if (!auth_off)
            {
                ...
                sprintf(log_buf, "%s!%s@%s a %s %s %s %s needs be to AUTHORIZED.",
                        ...
                        IS_GOOD(ch) ? "good" : IS_EVIL(ch) ? "evil"
                                                           : "neutral",
                        ch->sex == SEX_MALE ? "male" : ch->sex == SEX_FEMALE ? "female"
                                                                             : "neuter",
    ...
    "  May your visit here be tempest tossed.\n\r",
        ch ); */
            ...
            if (!IS_SET(ch->act, PLR_WIZINVIS) && !IS_SET(ch->act, PLR_CLOAKED))
                info("%s has returned to the storm.", (long)(ch->name), 0);
...
bool check_reconnect(DESCRIPTOR_DATA *d, char *name, bool fConn)
{
    ...
    {
        if (!IS_NPC(ch) && !ch->deleted && (!fConn || !ch->desc) && !str_cmp(d->character->name, ch->name))
        {
            if (fConn == FALSE)
            {
		 ...
		 */
                if ((obj = get_eq_char(ch, WEAR_LIGHT)) && obj->item_type == ITEM_LIGHT && obj->value[2] != 0 && ch->in_room)
                    ++ch->in_room->light;
            }
            return TRUE;
        }
...
========================================================================= */
                                                    /* - The heart of the
pager.  Thanks to N'Atas-Ha, ThePrincedom for porting */
                                                    /* this SillyMud code for MERC 2.0 and
laying down the groundwork.  */
                                                    /* - Thanks to Blackstar, hopper.cs.uiowa.edu 4000 for which the
improvements*/
/* to the pager was modeled from.  - Kahn */        /* - Safer, allows very large pagelen
now, and allows to page while switched */
...
void do_authorize(CHAR_DATA *ch, char *argument)
{
    ...
    for (d = descriptor_list; d; d = d_next)
    {
        ...
        if (mode == 0)
        {
            if (d->connected >= CON_AUTHORIZE_NAME && d->connected <= CON_AUTHORIZE_LOGOUT)
            {
                sprintf(buf, "  %s!%s@%s a %s %s %s %s\n\r",
                        d->character->name, d->user, d->host,
                        IS_GOOD(d->character) ? "good" : IS_EVIL(d->character) ? "evil"
                                                                               : "neutral",
                        d->character->sex == SEX_MALE ? "male" : d->character->sex == SEX_FEMALE ? "female"
                                                                                                 : "neuter",
        ...
        else if (d->connected >= CON_AUTHORIZE_NAME && d->connected <= CON_AUTHORIZE_LOGOUT
...
void send_to_area(AREA_DATA *pArea, char *txt)
{
    ...
    for (d = descriptor_list; d; d = d->next)
    {
        ...
        if (d->character)
        {
            if (!(d->character->in_room) || (d->character->deleted) || !IS_OUTSIDE(d->character) || !IS_AWAKE(d->character))
                continue;
...
*/
        if ((!str_suffix(pban->name, dnew->host) && !pban->user)
            ...
            || (pban->user && !str_cmp(pban->user, dnew->user) && !str_suffix(pban->name, dnew->host)))
        {
...
void do_hotreboot(CHAR_DATA *ch, char *argument)
{
    ...
    for (d = descriptor_list; d; d = d_next)
    {
        ...
        {
            {
                char *p; /* space-free for recover parse */
                for (p = d->user; *p; p++)
                    if (*p == ' ')
                        *p = '_';
                for (p = d->host; *p; p++)
...
void hotreboot_recover()
{
    ...
    for (;;)
    {
        ...
        descriptor_list = d;
        d->connected = CON_HOTREBOOT_RECOVER; /* so close_socket frees the char */
