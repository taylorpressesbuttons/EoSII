...
bool spec_guard(CHAR_DATA *ch)
{
    ...
    for (victim = ch->in_room->people; victim; victim = victim->next_in_room)
    {
        ...
        if (victim->fighting && (IS_GOOD(ch) || IS_NEUTRAL(ch)) && victim->fighting != ch && victim->alignment < max_evil)
        {
            max_evil = victim->alignment;
            ech = victim;
        }
        if (victim->fighting && IS_EVIL(ch) && victim->fighting != ch && victim->alignment > max_good)
        {
    ...
    if (victim)
    {
        ...
        snprintf(buf, sizeof(buf), "%s is a %s!  JUSTICE WILL PREVAIL!! DEATH TO %s!!!",
