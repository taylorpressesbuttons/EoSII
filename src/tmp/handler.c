...
*/
     ...
     */
    if ((wield = get_eq_char(ch, WEAR_WIELD)) && get_obj_weight(wield) > str_app[get_curr_str(ch)].wield)
    {
        static int depth;
...
CHAR_DATA *get_pc_world(CHAR_DATA *ch, char *argument)
{
    ...
    {
        if (d->connected == CON_PLAYING && is_name(ch, argument, d->character->name) && can_see(ch, d->character))
            return d->character;
    }
    return NULL;
}
...
OBJ_DATA *get_obj_carry(CHAR_DATA *ch, char *argument)
{
    ...
    for (obj = ch->carrying; obj; obj = obj->next_content)
    {
        if (obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj) && is_name(ch, arg, obj->name))
        {
            if (++count == number)
                return obj;
        }
...
OBJ_DATA *create_money(MONEY_DATA *amount)
{
    ...
    else if ((amount->silver == 1) && (amount->gold <= 0) && (amount->copper <= 0))
        obj = create_object(get_obj_index(OBJ_VNUM_MONEY_ONE), 0);
    else if ((amount->copper == 1) && (amount->gold <= 0) && (amount->silver <= 0))
        obj = create_object(get_obj_index(OBJ_VNUM_MONEY_ONE), 0);
    else
...
bool room_is_dark(ROOM_INDEX_DATA *pRoomIndex)
{
    ...
    if (weather_info.sunlight == SUN_DUSK || weather_info.sunlight == SUN_DARK)
        return TRUE;
...
bool can_see(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    if (!IS_NPC(victim) && IS_SET(victim->act, PLR_CLOAKED) && get_trust(ch) < victim->cloaked
        && (ch->in_room->vnum != victim->in_room->vnum))
        return FALSE;

    if (!IS_NPC(ch) && IS_SET(ch->act, PLR_HOLYLIGHT))
        return TRUE;
    if (is_raffected(ch->in_room, gsn_globedark) && ch->race != RACE_DROW)
        return FALSE;

    if (IS_AFFECTED(ch, AFF_BLIND) && !IS_AFFECTED2(ch, AFF_TRUESIGHT) && ch->race != RACE_ILLITHID)
        return FALSE;

    if (room_is_dark(ch->in_room) && !IS_AFFECTED(ch, AFF_INFRARED)
        && !IS_AFFECTED2(ch, AFF_TRUESIGHT) && (ch->race != RACE_ELF) && (ch->race != RACE_DWARF)
        && (ch->race != RACE_DROW) && (ch->race != RACE_DEMON))
        return FALSE;
    ...
    if (IS_AFFECTED(victim, AFF_INVISIBLE) && !IS_AFFECTED(ch, AFF_DETECT_INVIS)
        && !IS_AFFECTED2(ch, AFF_TRUESIGHT) && (ch->race != RACE_DEMON)
        && (!is_class(ch, CLASS_THIEF) && ch->level < 30))
        return FALSE;

    if (IS_AFFECTED(victim, AFF_HIDE) && !IS_AFFECTED(ch, AFF_DETECT_HIDDEN)
        && !IS_AFFECTED2(ch, AFF_TRUESIGHT) && !victim->fighting && ch->race != RACE_DWARF
        && ch->race != RACE_DEMON)
        return FALSE;
...
char *anticlass_bit_name(int anticlass)
{
    static char buf[512];
    buf[0] = '\0';
    if (anticlass & ITEM_ANTI_MAGE)
        strcat(buf, " anti-mage");
    if (anticlass & ITEM_ANTI_CLERIC)
        strcat(buf, " anti-cleric");
    if (anticlass & ITEM_ANTI_THIEF)
        strcat(buf, " anti-thief");
    if (anticlass & ITEM_ANTI_WARRIOR)
        strcat(buf, " anti-warrior");
    if (anticlass & ITEM_ANTI_PSI)
        strcat(buf, " anti-psi");
    if (anticlass & ITEM_ANTI_DRUID)
        strcat(buf, " anti-druid");
    if (anticlass & ITEM_ANTI_PALADIN)
        strcat(buf, " anti-paladin");
    if (anticlass & ITEM_ANTI_RANGER)
        strcat(buf, " anti-ranger");
    if (anticlass & ITEM_ANTI_BARD)
        strcat(buf, " anti-bard");
    if (anticlass & ITEM_ANTI_VAMP)
        strcat(buf, " anti-vampire");
    if (anticlass & ITEM_ANTI_NECRO)
        strcat(buf, " anti-necromancer");
    if (anticlass & ITEM_ANTI_WWF)
        strcat(buf, " anti-werewolf");
    if (anticlass & ITEM_ANTI_MONK)
...
char *antirace_bit_name(int antirace)
{
    ...
    if (antirace & ITEM_ANTI_PIXIE)
        strcat(buf, " anti-pixie");
    if (antirace & ITEM_ANTI_HALFLING)
        strcat(buf, " anti-halfling");
    if (antirace & ITEM_ANTI_DROW)
        strcat(buf, " anti-drow");
    if (antirace & ITEM_ANTI_ELDER)
        strcat(buf, " anti-elder");
    if (antirace & ITEM_ANTI_OGRE)
        strcat(buf, " anti-ogre");
    if (antirace & ITEM_ANTI_LIZARDMAN)
        strcat(buf, " anti-lizardman");
    if (antirace & ITEM_ANTI_DEMON)
        strcat(buf, " anti-demon");
    if (antirace & ITEM_ANTI_GHOUL)
        strcat(buf, " anti-ghoul");
    if (antirace & ITEM_ANTI_ILLITHID)
        strcat(buf, " anti-illithid");
    if (antirace & ITEM_ANTI_MINOTAUR)
        strcat(buf, " anti-minotaur");
    if (antirace & ITEM_ANTI_TROLL)
        strcat(buf, " anti-troll");
    if (antirace & ITEM_ANTI_SHADOW)
        strcat(buf, " anti-shadow");
    return (buf[0] != '\0') ? buf + 1 : "none";
}
...
char *act_bit_name(int act)
{
    ...
    else
    {
        ...
        if (act & PLR_AFK)
            strcat(buf, " AFK");
        if (act & PLR_BOUGHT_PET)
            strcat(buf, " boughtpet");
        if (act & PLR_AUTOEXIT)
            strcat(buf, " autoexit");
        if (act & PLR_AUTOLOOT)
            strcat(buf, " autoloot");
        ...
        if (act & PLR_ANSI)
            strcat(buf, " ansi");
        if (act & PLR_AUTOCOINS)
            strcat(buf, " autocoins");
        if (act & PLR_AUTOSPLIT)
...
{ /* It ain't pretty, but it works :) -- Hannibal */
    if (code == 'r' || code == 'R' || code == 'b' || code == 'B' || code == 'g' || code == 'G'
        || code == 'w' || code == 'W' || code == 'p' || code == 'P' || code == 'Y' || code == 'O'
        || code == 'c' || code == 'C' || code == 'z' || code == '.')
        return TRUE;
    return FALSE;
}
