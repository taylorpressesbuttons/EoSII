...
void do_quest(CHAR_DATA *ch, char *argument)
{
    ...
    else if (!strcmp(arg1, "complete"))
    {
        ...
        if (IS_SET(ch->act, PLR_QUESTOR))
        {
            ...
            else if ((ch->questmob || ch->questobj) && ch->countdown > 0)
            {
                sprintf(buf, "You haven't completed the quest yet, but there is still time!");
                do_say(questman, buf);
                return;
...
void generate_quest(CHAR_DATA *ch, CHAR_DATA *questman)
{
    ...
    for (vsearch = char_list; vsearch; vsearch = vsearch_next)
    {
        ...
        if ((level_diff <= 3 && level_diff > -5) && (IS_NPC(vsearch))
            ...
            && (ch->level > vsearch->in_room->area->llevel) && (!IS_SET(vsearch->act, ACT_TRAIN))
...
void quest_update(void)
{
    ...
    for (ch = char_list; ch; ch = ch_next)
    {
        ...
        {
            if (ch->questmob && ch->questmob->deleted && ch->countdown > 1)
            {
