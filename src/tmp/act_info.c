...
void show_char_to_char_0(CHAR_DATA *victim, CHAR_DATA *ch)
{
    ...
    if (IS_AFFECTED(victim, AFF_HIDE))
        strcat(buf, "(Hide) ");
    if (IS_AFFECTED(victim, AFF_CHARM) && (!IS_SET(victim->act, UNDEAD_TYPE(victim))))
        strcat(buf, "(Charmed) ");
...
void show_char_to_char_1(CHAR_DATA *victim, CHAR_DATA *ch, char *argument)
{
   ...
   and then return w/o showing char stuff   --Angi */
    ...
    if (percent >= 100)
        ...
        send_to_char((AT_BLOOD + AT_BLINK), buf, ch);
            ...
            send_to_char(AT_CYAN, "\n\r", ch);
                 ...
                 && ((iWear == WEAR_WIELD && !get_eq_char(victim, iWear))
                     || (iWear == WEAR_WIELD_2 && !get_eq_char(victim, iWear)
                         && !get_eq_char(victim, WEAR_HOLD) && !get_eq_char(victim, WEAR_SHIELD)
...
void show_char_to_char(CHAR_DATA *list, CHAR_DATA *ch)
{
    ...
    for (rch = list; rch; rch = rch->next_in_room)
    {
        ...
        if (!(rch->desc) && !IS_NPC(rch) && get_trust(ch) < L_APP)
            continue;

        if (!IS_NPC(rch) && IS_SET(rch->act, PLR_WIZINVIS) && get_trust(ch) < rch->wizinvis)
            continue;
...
void do_look(CHAR_DATA *ch, char *argument)
{
    ...
    if (is_raffected(ch->in_room, gsn_globedark) && !IS_SET(ch->act, PLR_HOLYLIGHT) && ch->race != RACE_DROW)
    {
    ...
    if (!IS_NPC(ch) && !IS_SET(ch->act, PLR_HOLYLIGHT) && room_is_dark(ch->in_room)
        && (ch->race != RACE_ELF) && (ch->race != RACE_DWARF) && (ch->race != RACE_DROW)
        && (ch->race != RACE_DEMON) && (ch->race != RACE_TABAXI))
    {
...
void do_scry_exits(CHAR_DATA *ch, ROOM_INDEX_DATA *scryer)
{
    ...
    {
        if ((pexit = scryer->exit[door]) && pexit->to_room && !IS_SET(pexit->exit_info, EX_CLOSED))
        {
            found = TRUE;
            if (fAuto)
            {
        ...
        if ((pexit = ch->in_room->exit[door]) && pexit->to_room)
        {
            ...
            else
            {
                if (fAuto)
                {
                    strcat(buf, " &O[&P ");
                    strcat(buf, dir_name[door]);
                    strcat(buf, " &Rclosed &O]");
                }
                else
                {
                    sprintf(buf + strlen(buf), "&O%-5s&w - [&RClosed&w]\n\r",
                            capitalize(dir_name[door]));
                }
            }
        }
...
void do_score(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && ch->pcdata->condition[COND_THIRST] == 0 && ch->level >= LEVEL_IMMORTAL)
        send_to_char(AT_BLUE, "You are dying of thirst.\n\r", ch);
    if (!IS_NPC(ch) && ch->pcdata->condition[COND_FULL] == 0 && ch->level >= LEVEL_IMMORTAL)
    ...
    if (GET_AC(ch) >= 101)
        ...
        send_to_char(AT_GREEN, "wearing "
                               "clothes.\n\r",
                     ch);
    else if (GET_AC(ch) >= -50)
        send_to_char(AT_GREEN, "slightly "
                               "armored.\n\r",
                     ch);
    else if (GET_AC(ch) >= -100)
        send_to_char(AT_GREEN, "somewhat "
                               "armored.\n\r",
                     ch);
        ...
        send_to_char(AT_GREEN, "well "
                               "armored.\n\r",
                     ch);
    else if (GET_AC(ch) >= -750)
        send_to_char(AT_GREEN, "strongly "
                               "armored.\n\r",
                     ch);
    else if (GET_AC(ch) >= -1000)
        send_to_char(AT_GREEN, "heavily "
                               "armored.\n\r",
                     ch);
    else if (GET_AC(ch) >= -1200)
        send_to_char(AT_GREEN, "superbly "
                               "armored.\n\r",
                     ch);
    else if (GET_AC(ch) >= -1400)
        send_to_char(AT_GREEN, "divinely "
                               "armored.\n\r",
                     ch);
    else
        send_to_char(AT_GREEN, "invincible!\n\r", ch);
    ...
    if (ch->alignment > 900)
        send_to_char(AT_BLUE, "angelic.\n\r", ch);
    else if (ch->alignment > 700)
        send_to_char(AT_BLUE, "saintly.\n\r", ch);
    else if (ch->alignment > 350)
        send_to_char(AT_BLUE, "good.\n\r", ch);
    else if (ch->alignment > 100)
        send_to_char(AT_BLUE, "kind.\n\r", ch);
    else if (ch->alignment > -100)
        send_to_char(AT_YELLOW, "neutral.\n\r", ch);
    else if (ch->alignment > -350)
        send_to_char(AT_RED, "mean.\n\r", ch);
    else if (ch->alignment > -700)
        send_to_char(AT_RED, "evil.\n\r", ch);
    else if (ch->alignment > -900)
        send_to_char(AT_RED, "demonic.\n\r", ch);
    else
        send_to_char(AT_RED, "satanic.\n\r", ch);
...
void do_help(CHAR_DATA *ch, char *argument)
{
    ...
    for (pHelp = help_first; pHelp; pHelp = pHelp->next)
    {
        ...
        {
            if (pHelp->level >= 0 && str_cmp(argument, "imotd") && str_prefix("advm_", argument)
 ...
 */
void build_who_string(CHAR_DATA *ch, char *buf3, char *argument)
{
    DESCRIPTOR_DATA *d;
    ...
    for (d = descriptor_list; d; d = d->next)
    {
        ...
        if (wch->level < iLevelLower || wch->level > iLevelUpper
            ...
            || (fHeroOnly && wch->level < LEVEL_HERO) || (fHeroOnly && wch->level > L_CHAMP5)
...
guild_table[iGuild].name ) )   */
        ...
        if (strlen(buf3) > (MAX_STRING_LENGTH * 9) - MAX_STRING_LENGTH) /* buffer cap */
            break;
        ...
        if (wch->level >= L_APP && wch->pcdata->whotype && str_cmp(wch->pcdata->whotype, "!!!!!!!!!!!!"))
        ...
        else if (wch->level >= LEVEL_HERO && wch->pcdata->whotype && !strcmp(class, wch->pcdata->whotype))
        {
        ...
        if (wch->guild != NULL)
        {
            ...
            sprintf(buf3 + strlen(buf3), "&%c", colors[wch->guild->color]);
            strcat(buf3, buf);
            buf[0] = '\0';
            switch (wch->guild_rank)
            {
                case 0:
                    ...
                    break;
                case 1:
                    sprintf(buf + strlen(buf), " %s] ", wch->sex == SEX_FEMALE ? "Lady" : "Lord");
                    break;
                case 2:
        ...
        if (wch->desc && wch->desc->editor != 0)
        {
            ...
            {
                strcat(buf3, "&z");
                strcat(buf3, "&z[&OForging&z] ");
            }
            else
            {
                strcat(buf3, "&W");
                strcat(buf3, "&R<&CBuilding&R> ");
            }
        }
    ...
    {
        strcat(buf3, "&G");
        strcat(buf3, "There is no class/race/clan/person by that name in the game.\n\r");
    }
    return;
}
...
{
    char buf[MAX_STRING_LENGTH * 9];
    build_who_string(ch, buf, argument);
    if (buf[0] != '\0')
            ...
            if (is_class(ch, CLASS_WEREWOLF)
                && ((iWear == WEAR_WIELD && !get_eq_char(ch, iWear))
                    || (iWear == WEAR_WIELD_2 && !get_eq_char(ch, iWear)
                        && !get_eq_char(ch, WEAR_HOLD) && !get_eq_char(ch, WEAR_SHIELD)
...
void do_where(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0')
    {
        ...
        for (d = descriptor_list; d; d = d->next)
        {
            if (d->connected == CON_PLAYING && (victim = d->character) && !IS_NPC(victim)
                && victim->in_room
                && (victim->in_room->area == ch->in_room->area || ch->level >= LEVEL_IMMORTAL)
                && can_see(ch, victim))
            {
                found = TRUE;
                if (ch->level >= LEVEL_IMMORTAL)
                    sprintf(buf, "%-18s [%c][%5d] %s\n\r",
                            victim->name, victim->fighting ? 'F' : (victim->desc && victim->desc->editor != 0) ? 'E'
    ...
    else
    {
        ...
        for (victim = char_list; victim; victim = victim->next)
        {
            if (((!victim->in_room || IS_AFFECTED(victim, AFF_HIDE)
                  || IS_AFFECTED(victim, AFF_SNEAK) || victim->race == RACE_HALFLING)
                 && ch->level < LEVEL_IMMORTAL)
                || ((ch->level >= LEVEL_IMMORTAL) && (!victim->in_room)))
                continue;

            if (((victim->in_room->area == ch->in_room->area) || (ch->level >= LEVEL_IMMORTAL))
                && can_see(ch, victim) && is_name(ch, arg, victim->name))
            {
                found = TRUE;
                if (ch->level >= LEVEL_IMMORTAL)
                    sprintf(buf, "%-18s [%c][%5d] %s\n\r",
                            PERS(victim, ch), victim->fighting ? 'F' : (victim->desc && victim->desc->editor != 0) ? 'E'
                                                                                                                   : ' ',
                            victim->in_room->vnum, victim->in_room->name);
                else
                    sprintf(buf, "%-28s %s\n\r",
...
void do_practice(CHAR_DATA *ch, char *argument)
{
    ...
    else
    {
        ...
        if ((sn = skill_lookup(argument)) < 0 || (!IS_NPC(ch) && !can_use_skpell(ch, sn)))
        {
...
void do_channels(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0')
    {
        ...
        send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_AUCTION) ? " +AUCTION" : " -auction",
                     ch);

        send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_CHAT) ? " +CHAT" : " -chat",
                     ch);

        send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_OOC) ? " +OOC" : " -ooc",
                     ch);

        send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_CLASS) ? " +CLASS" : " -class",
                     ch);

        send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_CLAN) ? " +CLAN" : " -clan",
                     ch);
        /* 	if( ch->level > 2 )
...
*/
        if (IS_HERO(ch)) /* XOR */
        {
            send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_HERO) ? " +HERO" : " -hero", ch);
        }

        if (IS_IMMORTAL(ch))
        {
            send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_IMMTALK) ? " +IMMTALK" : " -immtalk",
                         ch);
            send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_IMC) ? " +IMC" : " -imc",
                         ch);
        }

        send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_MUSIC) ? " +MUSIC" : " -music",
                     ch);

        send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_QUESTION) ? " +QUESTION" : " -question",
                     ch);

        send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_SHOUT) ? " +SHOUT" : " -shout",
                     ch);

        send_to_char(AT_PINK, !IS_SET(ch->deaf, CHANNEL_YELL) ? " +YELL" : " -yell",
                     ch);

        send_to_char(AT_LBLUE, !IS_SET(ch->deaf, CHANNEL_INFO) ? " +INFO" : " -info", ch);

        send_to_char(AT_PURPLE, !IS_SET(ch->deaf, CHANNEL_CHALLENGE) ? " +CHALLENGE" : " -challenge", ch);

        send_to_char(AT_RED, !IS_SET(ch->deaf, CHANNEL_ARENA) ? " +ARENA" : " -arena", ch);
	 ...
	 */
        ...
        {
            send_to_char(AT_DGREY, !IS_SET(ch->deaf, CHANNEL_LOG) ? " +LOG" : " -log",
                         ch);
        }

        if (get_trust(ch) >= L_SEN)
        {
            send_to_char(AT_DGREY, !IS_SET(ch->deaf, CHANNEL_BUILD) ? " +BUILD" : " -build",
                         ch);
        }

        if (get_trust(ch) >= L_DIR)
        {
            send_to_char(AT_DGREY, !IS_SET(ch->deaf, CHANNEL_GOD) ? " +GOD" : " -god",
                         ch);
        }

        if (get_trust(ch) >= L_IMP)
        {
            send_to_char(AT_DGREY, !IS_SET(ch->deaf, CHANNEL_GUARDIAN) ? " +GUARD" : " -guard",
                         ch);
        }
        ...
        if (get_trust(ch) >= L_IMP)
        {
            send_to_char(AT_DGREY, !IS_SET(ch->deaf, CHANNEL_CLASS_MASTER) ? " +CLASSMASTER" : " -classmaster",
                         ch);
        }

        if (get_trust(ch) >= L_IMP)
        {
            send_to_char(AT_DGREY, !IS_SET(ch->deaf, CHANNEL_CLAN_MASTER) ? " +CLANMASTER" : " -clanmaster",
                         ch);
        }

        if (get_trust(ch) >= L_IMP)
        {
            send_to_char(AT_DGREY, !IS_SET(ch->deaf, CHANNEL_GUILD_MASTER) ? " +GUILDMASTER" : " -guildmaster",
                         ch);
        }
...
void do_config(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0')
    {
        ...
        send_to_char(AT_RED, IS_SET(ch->act, PLR_AUTOEXIT) ? "&w[&Y+AUTOEXIT  &w]&W You automatically see exits.\n\r" : "&w[&Y-autoexit  &w]&W You don't automatically see exits.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_AUTOCOINS) ? "&w[&Y+AUTOCOINS &w]&W You automatically get coins from corpses.\n\r" : "&w[&Y-autocoins &w]&W You don't automatically get coins from corpses.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_AUTOSPLIT) ? "&w[&Y+AUTOSPLIT &w]&W You automatically split coins with group members.\n\r" : "&w[&Y-autosplit &w]&W You don't automatically split coins with group members.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_AUTOLOOT) ? "&w[&Y+AUTOLOOT  &w]&W You automatically loot corpses.\n\r" : "&w[&Y-autoloot  &w]&W You don't automatically loot corpses.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_AUTOSAC) ? "&w[&Y+AUTOSAC   &w]&W You automatically sacrifice corpses.\n\r" : "&w[&Y-autosac   &w]&W You don't automatically sacrifice corpses.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_ANSI) ? "&w[&Y+ANSI      &w]&W You have ansi color enabled.\n\r" : "&w[&Y-ansi      &w]&W You have ansi color disabled.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_BLANK) ? "&w[&Y+BLANK     &w]&W You have a blank line before your prompt.\n\r" : "&w[&Y-blank     &w]&W You have no blank line before your prompt.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_BRIEF) ? "&w[&Y+BRIEF     &w]&W You see brief descriptions.\n\r" : "&w[&Y-brief     &w]&W You see long descriptions.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_FULLNAME) ? "&w[&Y+FULLNAME  &w]&W You have name completion off.\n\r" : "&w[&Y-fullname  &w]&W You are using name completion.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_COMBINE) ? "&w[&Y+COMBINE   &w]&W You see object lists in combined format.\n\r" : "&w[&Y-combine   &w]&W You see object lists in single format.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_PROMPT) ? "&w[&Y+PROMPT    &w]&W You have a prompt.\n\r" : "&w[&Y-prompt    &w]&W You don't have a prompt.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_TELNET_GA) ? "&w[&Y+TELNETGA  &w]&W You receive a telnet GA sequence.\n\r" : "&w[&Y-telnetga  &w]&W You don't receive a telnet GA sequence.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_COMBAT) ? "&w[&Y+COMBAT    &w]&W You see all combat scroll.\n\r" : "&w[&Y-combat    &w]&W You do not see dodge/parry/miss in combat.\n\r", ch);

        send_to_char(AT_RED, IS_SET(ch->act, PLR_SILENCE) ? "&w[&Y+SILENCE   &w]&W You are silenced.\n\r" : "", ch);

        send_to_char(AT_RED, !IS_SET(ch->act, PLR_NO_EMOTE) ? "" : "&w[&Y-emote    &w]&W You can't emote.\n\r", ch);

        send_to_char(AT_RED, !IS_SET(ch->act, PLR_NO_TELL) ? "" : "&w[&Y-tell     &w]&W You can't use 'tell'.\n\r", ch);
    }
    else
    {
        char buf[MAX_STRING_LENGTH];
...
void do_spells(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_NPC(ch) || (!IS_NPC(ch) && !has_spells(ch)))
    {
...
void do_slist(CHAR_DATA *ch, char *argument)
{
    ...
    for (level = 1; level <= L_CHAMP3; level++)
    {
        ...
        for (sn = 0; skill_table[sn].name[0] != '\0'; sn++)
        {
            if (!skill_table[sn].name)
                break;
            if ((onlyCl || classes == 1) && skill_table[sn].skill_level[profession] != level)
                continue;

            if (classes == 2 && skill_table[sn].skill_level[profession] != level
                && skill_table[sn].skill_level[ch->class[1]] != level)
                continue;

            if (classes == 3 && skill_table[sn].skill_level[profession] != level
                && skill_table[sn].skill_level[ch->class[1]] != level
...
void do_guild(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(victim->act, PLR_THIEF) && !strcmp(ch->guild->name, "MERCENARY"))
    {
...
void do_guilds(CHAR_DATA *ch, char *argument)
{
    ...
    for (cnt = 0; guild_table[cnt].name[0] != '\0'; cnt++)
    {
        sprintf(buf, "&z[&W%*s&z] [&W%*s&z]\n\r",
                ...
                (int)(20 + strlen(guild_table[cnt].deity) - strlen_wo_col(guild_table[cnt].deity)),
...
void do_clanscore(CHAR_DATA *ch, char *argument)
{
    CLAN_DATA *pClan;
    char buf[MAX_STRING_LENGTH];
    char clanlev[MAX_INPUT_LENGTH];

    if (IS_NPC(ch))
    {
        send_to_char(AT_WHITE, "You lead yourself grandly!\n\r", ch);
        return;
    }

    if (ch->clan == 0 || !(pClan = get_clan_index(ch->clan)))
    {
        send_to_char(AT_WHITE, "You are unclanned.\n\r", ch);
        return;
    }
    ...
    {
        case 1:
            sprintf(clanlev, "Centurion");
            break;
        case 2:
            sprintf(clanlev, "Council");
            break;
        case 3:
            sprintf(clanlev, "Leader");
            break;
        case 4:
            sprintf(clanlev, "Champion");
            break;
        case 5:
            sprintf(clanlev, "Deity");
            break;
        default:
            sprintf(clanlev, "Member");
            break;
    }

    sprintf(buf, "\n\r&CYou are rank %d, %s of %s&C.\n\r", ch->clev, clanlev, pClan->name);
    send_to_char(C_DEFAULT, buf, ch);
    sprintf(buf, "&CYour clan has %d members.\n\r", pClan->members);
    send_to_char(C_DEFAULT, buf, ch);
    return;
}
...
#ifdef NEW_MONEY
    sprintf(log_buf, "&gYou are carrying &Y%d gold, &w%d silver &gand &O%d copper.\n\r", ch->money.gold,
...
{
    int play_time, day, hour, minute, second;
    unsigned char pcnt_tnl, pcnt_fll;

    if (IS_NPC(ch))
        return;

    play_time = (current_time - ch->logon);
    play_time += ch->played;
    day = (int)(play_time / 86400);
    hour = (int)((play_time - (day * 86400)) / 3600);
    minute = (int)(((play_time - (day * 86400)) - hour * 3600) / 60);
    second = (int)((((play_time - (day * 86400)) - hour * 3600) - minute * 60));

    send_to_char(AT_RED, "&W======================= Exp Related =======================\n\r", ch);

    if (ch->level < L_CHAMP3)
    {
        pcnt_fll = PERCENT_LEFT((xp_tolvl(ch)), ch->exp);
        pcnt_tnl = PERCENT((xp_tolvl(ch)), ch->exp);
        sprintf(log_buf, "&PYou have scored a total of %dxp.\n\rYou are %dxp off from your"
                         " next level.&z\n\r",
                ch->exp, xp_tolvl(ch) - ch->exp);
        send_to_char(AT_RED, log_buf, ch);
        sprintf(log_buf, "&CYou have earned %d%% exp towards your next level.\n\rYou are level %d\n\r", pcnt_fll, ch->level);
        send_to_char(AT_RED, log_buf, ch);
        sprintf(log_buf, "&CYou are %d%% from level %d&z\n\r", pcnt_tnl, ch->level + 1);
        send_to_char(AT_RED, log_buf, ch);
    }
    else
    {
    ...
    send_to_char(AT_RED, "&W========================= Stats ===========================\n\r", ch);
    sprintf(log_buf, "&WYour Current Str is %d. This includes a Race mod of %d.\n\r",
            get_curr_str(ch), (get_race_data(ch->race))->mstr);
    send_to_char(AT_WHITE, log_buf, ch);
    sprintf(log_buf, "&WYour Current Wis is %d. This includes a Race mod of %d.\n\r",
            get_curr_wis(ch), (get_race_data(ch->race))->mwis);
    send_to_char(AT_WHITE, log_buf, ch);
    sprintf(log_buf, "&WYour Current Int is %d. This includes a Race mod of %d.\n\r",
            get_curr_int(ch), (get_race_data(ch->race))->mint);
    send_to_char(AT_WHITE, log_buf, ch);
    sprintf(log_buf, "&WYour Current Dex is %d. This includes a Race mod of %d.\n\r",
            get_curr_dex(ch), (get_race_data(ch->race))->mdex);
    send_to_char(AT_WHITE, log_buf, ch);
    sprintf(log_buf, "&WYour Current Con is %d. This includes a Race mod of %d.\n\r",
            get_curr_con(ch), (get_race_data(ch->race))->mcon);
    send_to_char(AT_WHITE, log_buf, ch);

    send_to_char(AT_RED, "&W========================= Other ===========================\n\r", ch);
#ifdef NEW_MONEY
    sprintf(log_buf, "&CYou are carrying &Y%d Gold&C, &W%d Silver&C, and &O%d copper coins.\n\r", ch->money.gold,
            ch->money.silver, ch->money.copper);
#else
    sprintf(log_buf, "&YYou are currently carrying %d coins.\n\r", ch->gold);
#endif
    send_to_char(AT_RED, log_buf, ch);

    sprintf(log_buf, "&CYou have &W%d &Cquest points.\n\r", ch->questpoints);
    send_to_char(AT_WHITE, log_buf, ch);

    send_to_char(AT_WHITE, "&WYou have been playing &PEye of the &BStorm&W for:&z\n\r", ch);
    sprintf(log_buf, "&B%d Days, %d Hours, %d Minutes, and %d Seconds.\n\r", day, hour, minute, second);
    send_to_char(AT_WHITE, log_buf, ch);
