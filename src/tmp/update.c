...
void gain_exp(CHAR_DATA *ch, int gain)
{
    ...
    while (ch->exp >= xp_tolvl(ch))
    {
        ...
        info("%s advances to level %d!", (long)ch->name, ch->level);
        advance_level(ch);
    }
...
void gain_condition(CHAR_DATA *ch, int iCond, int value)
{
    ...
    hploss = (ch->level < 10) ? 1 : (ch->level < 20) ? 2
                                                     : 3;
...
void mobile_update(void)
{
    ...
    for (ch = char_list; ch; ch = ch->next)
    {
        ...
        if (!IS_NPC(ch) || !ch->in_room || IS_AFFECTED(ch, AFF_STUN) || IS_AFFECTED(ch, AFF_CHARM) || ch->wait > 0)
            continue;
        ...
        /* Wander */
        if (!IS_SET(ch->act, ACT_SENTINEL) && (door = number_bits(5)) <= 5
            && (pexit = ch->in_room->exit[door]) && pexit->to_room
            && !IS_SET(pexit->exit_info, EX_CLOSED)
            && !IS_SET(pexit->to_room->room_flags, ROOM_NO_MOB) && !ch->hunting
        ...
        /* Flee */
        if (ch->hit < (MAX_HIT(ch) / 2) && (door = number_bits(3)) <= 5
            && (pexit = ch->in_room->exit[door]) && pexit->to_room
            && !IS_SET(pexit->exit_info, EX_CLOSED)
...
void char_update(void)
{
	 ...
	 */
        if (!IS_NPC(ch) && (!ch->desc || ch->desc->connected == CON_PLAYING) && ch->level >= 2 && ch->save_time < save_time)
        {
            ch_save = ch;
        ...
        if (!IS_NPC(ch) && (ch->level < LEVEL_IMMORTAL || (!ch->desc && !IS_SWITCHED(ch))))
        {
            OBJ_DATA *obj;
...
void aggr_update(void)
{
    ...
    for (d = descriptor_list; d; d = d->next)
    {
        ...
        for (mch = ch->in_room->people; mch; mch = mch->next_in_room)
        {
            ...
            if (!IS_NPC(mch) || mch->deleted || IS_AFFECTED(mch, AFF_STUN)
                /*		|| !IS_SET( mch->act, ACT_AGGRESSIVE )*/
                || mch->fighting || IS_AFFECTED(mch, AFF_CHARM) || !IS_AWAKE(mch)
            ...
            if (IS_NPC(mch) && mch->mpactnum > 0 && mch->in_room->area->nplayer > 0)
            {
            ...
            if (!IS_SET(mch->act, ACT_AGGRESSIVE) || (IS_SET(mch->act, ACT_WIMPY) && IS_AWAKE(ch)) || IS_AFFECTED(ch, AFF_PEACE))
                continue;
	     ...
	     */
            ...
            {
                if (IS_NPC(vch) || vch->deleted || vch->level >= LEVEL_IMMORTAL)
                    continue;
                ...
                if (paf->deleted || ch->deleted)
                {
                    ...
                    else
                    {
                        ...
                        if (!prev)
                        {
                            ...
                            sprintf(log_buf, "Char: %s", ch->name ? ch->name : "(Unknown)");
                            bug(log_buf, 0);
                            continue;
                        }
                    }
