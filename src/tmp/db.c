            ...
            if (letter == 'D')
            {
                ...
                switch (locks) /* OLC exit_info to rs_flags. */
                {
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        pexit->rs_flags = EX_ISDOOR | EX_BASHPROOF | EX_PASSPROOF;
                        break;
                    case 8:
                        pexit->rs_flags = EX_ISDOOR | EX_PICKPROOF | EX_BASHPROOF | EX_PASSPROOF;
                        break;
                }
...
void fix_exits(void)
{
    ...
    for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
    {
             ...
             pRoomIndex = pRoomIndex->next)
        {
            for (door = 0; door <= 5; door++)
            {
                if ((pexit = pRoomIndex->exit[door]) && (to_room = pexit->to_room)
...
void reset_room(ROOM_INDEX_DATA *pRoom)
{
    ...
    {
        EXIT_DATA *pExit;
        if ((pExit = pRoom->exit[iExit]) && !IS_SET(pExit->exit_info, EX_BASHED)) /* Skip Bashed. */
        {
...
CHAR_DATA *create_mobile(MOB_INDEX_DATA *pMobIndex)
{
    ...
    mob->start_align = (IS_EVIL(mob) ? 'E' : IS_GOOD(mob) ? 'G'
                                                          : 'N');
    /*
     * Insert in list.
     */
...
{
    if (!pstr || pstr == &str_empty[0] || (pstr >= string_space && pstr < top_string))
        return;
 ...
 * Info channel.. -- Alty
 */
void info(const char *str, long param1, long param2)
{
    char buf[MAX_STRING_LENGTH];
    DESCRIPTOR_DATA *d;
...
void arena_chann(const char *str, long param1, long param2)
{
    char buf[MAX_STRING_LENGTH];
    DESCRIPTOR_DATA *d;
...
void challenge(const char *str, long param1, long param2)
{
    char buf[MAX_STRING_LENGTH];
    DESCRIPTOR_DATA *d;
...
void logch(char *l_str, int l_type, int lvl)
{
    ...
    for (d = descriptor_list; d; d = d->next)
    {
        if (d->connected != CON_PLAYING || IS_SET(d->character->deaf, l_type) || get_trust(d->character) < level ||
...
char *wind_str(int str)
{
    ...
    {
        case 1:
            return "impossibly strongly";
        case 2:
            return "very strongly";
        case 3:
            return "strongly";
        case 4:
            return "moderately strongly";
        case 5:
            return "lightly";
        default:
            return "wierdly";
...
void race_sort(RACE_DATA *pRace)
{
    ...
    {
        if (pRace->vnum == fRace->vnum || (pRace->vnum > fRace->vnum && (!fRace->next || pRace->vnum < fRace->next->vnum)))
        {
