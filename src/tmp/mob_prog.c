...
void mprog_greet_trigger(CHAR_DATA *ch)
{
    ...
    for (vmob = ch->in_room->people; vmob != NULL; vmob = vmob->next_in_room)
        if (IS_NPC(vmob) && ch != vmob && can_see(vmob, ch) && (vmob->fighting == NULL)
            && IS_AWAKE(vmob) && (vmob->pIndexData->progtypes & GREET_PROG))
        {
            mprog_percent_check(vmob, ch, NULL, NULL, GREET_PROG);
        }
        else if (IS_NPC(vmob) && ch != vmob && (vmob->fighting == NULL) && IS_AWAKE(vmob)
...
void mprog_hitprcnt_trigger(CHAR_DATA *mob, CHAR_DATA *ch)
{
    ...
    {
        for (mprg = mob->pIndexData->mobprogs; mprg != NULL; mprg = mprg->next)
            if ((mprg->type & HITPRCNT_PROG) && ((100 * mob->hit / MAX_HIT(mob)) < atoi(mprg->arglist)))
            {
