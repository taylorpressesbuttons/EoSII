...
void do_empower(CHAR_DATA *ch, char *argument)
{
    ...
    if (!is_name(NULL, "empower", ch->pcdata->empowerments) && ch->level != L_IMP)
    {
...
void do_detract(CHAR_DATA *ch, char *argument)
{
    ...
    if (!is_name(NULL, "detract", ch->pcdata->empowerments) && ch->level != L_IMP)
    {
...
void do_astat(CHAR_DATA *ch, char *argument)
{
    ...
    sprintf(buf, "&WSounds:   &w%s\n\r", pArea->reset_sound ? pArea->reset_sound : "(default)");
    send_to_char(C_DEFAULT, buf, ch);
...
void do_ofind(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '*')
    {
        ...
        if (str_cmp((arg + 1), "invoke"))
        {
            ...
            if (type == ITEM_ARMOR && sub_type < 1)
            {
                send_to_char(AT_WHITE, "Unknow armor sub-type.\n\r", ch);
                return;
            }
            if (type == ITEM_WEAPON && !str_cmp(flag_string(weapon_flags, sub_type), "none"))
            {
            ...
            for (vnum = 0; nMatch < top_obj_index; vnum++)
            {
                ...
                {
                    nMatch++;
                    if (type == ITEM_ARMOR && pObjIndex->item_type == type && CAN_WEAR(pObjIndex, sub_type))
                    {
                        found = TRUE;
                    ...
                    else if (type == ITEM_WEAPON && pObjIndex->item_type == type && pObjIndex->value[3] == sub_type)
                    {
                        found = TRUE;
                        sprintf(buf, "[&W%5d&z] &w%s\n\r",
                                pObjIndex->vnum, capitalize(pObjIndex->short_descr));
                    ...
                    else if (pObjIndex->item_type == type && !sub_type)
                    {
                        found = TRUE;
                        sprintf(buf, "[&W%5d&z] &w%s\n\r",
                                pObjIndex->vnum, capitalize(pObjIndex->short_descr));
...
void do_mwhere(CHAR_DATA *ch, char *argument)
{
    ...
    {
        if (IS_NPC(victim) && victim->in_room && is_name(ch, arg, victim->name))
        {
            found = TRUE;
...
void ban(CHAR_DATA *ch, char *argument, char ban_type)
{
    ...
    if (arg[0] == '\0')
    {
        ...
        sprintf(buf, "Syntax: %s site.to.ban [username]\n\r",
                ban_type == 'P' ? "Permban" : ban_type == 'T' ? "Tempban"
                                                              : "Newban");
        send_to_char(C_DEFAULT, buf, ch);
        send_to_char(C_DEFAULT,
...
void do_cset(CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(arg2, "clvl") && (is_name(NULL, "cset", ch->pcdata->empowerments) || ch->level == L_IMP))
    {
        if IS_NPC (victim)
        {
            send_to_char(AT_WHITE, "Not on NPC's.\n\r", ch);
...
*/
    if (!str_cmp(arg2, "act"))
    {
        ...
        if (IS_NPC(victim))
        {
            ...
            if (!str_cmp(arg3, "gambler") || !str_cmp(arg3, "g"))
                value = 2048;
            if ((!str_cmp(arg3, "prototype") || !str_cmp(arg3, "pro")) && (get_trust(ch) >= L_CON
    ...
    if (!str_cmp(arg2, "class"))
    {
        ...
        {
            if (!str_prefix(arg4, class_table[iClass].who_long) || !str_cmp(arg4, class_table[iClass].who_name))
            {
                value = iClass;
                break;
            }
    ...
    if (!str_cmp(arg2, "race"))
    {
        ...
        {
            if (!str_cmp(arg3, (get_race_data(iRace))->race_full) || !str_cmp(arg3, (get_race_data(iRace))->race_name))
            {
                value = iRace;
                break;
            }
...
void do_users(CHAR_DATA *ch, char *argument)
{
        ...
        for (d = descriptor_list; d; d = d->next)
        {
            if (d->character && can_see(ch, d->character))
            {
                ...
                sprintf(buf + strlen(buf), "[%4d %-11s] %s&W!&B%s&W@&G%s&R\n\r",
                        ...
                        d->original ? d->original->name : d->character ? d->character->name
                                                                       : "(none)",
                        d->user ? d->user : "(none)",
                        d->host);
            }
    ...
    else /* To be able to do "user <name>" -Angi */
    {
        for (d = descriptor_list; d; d = d->next)
        {
            if (d->character && can_see(ch, d->character))
            {
                if (!str_cmp(arg, d->original ? d->original->name : d->character ? d->character->name
                                                                                 : "(none)"))
                {
                    count++;
                    sprintf(buf + strlen(buf), "[%4d %-11s] %s&W!&B%s&W@&G%s&R\n\r",
...
void do_force(CHAR_DATA *ch, char *argument)
{
    ...
    for (cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++)
    {
        if (argument[0] == cmd_table[cmd].name[0] && !str_prefix(argument, cmd_table[cmd].name)
    ...
    if (!str_cmp(arg, "all") || is_number(arg))
    {
        ...
        for (vch = char_list; vch; vch = vch->next)
        {
            ...
            if (!IS_NPC(vch) && get_trust(vch) < get_trust(ch) && (vch->level >= llvlr && vch->level <= hlvlr))
            {
        ...
        for (obj = object_list; obj; obj = obj->next)
        {
            ...
            else
            {
                sprintf(buf, "[%2d] %s in %s at [%4d].\n\r", obj_counter,
                        obj->short_descr, (!in_obj->in_room) ? "somewhere" : in_obj->in_room->name,
...
void do_restrict(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg[0] == '\0' || arg1[0] == '\0' || (!is_number(arg1) && ((arg2[0] == '\0') || !is_number(arg2))))
    {
    ...
    if (((lvl < 0 || lvl > L_IMP || lvl > get_trust(ch)) && (arg2[0] == '\0')) || ((arg2[0] != '\0') && (lvl < 0 || lvl > 3)))
    {
        send_to_char(AT_WHITE, "Invalid level.\n\r", ch);
        return;
    }

    for (cmd = 1; cmd_table[cmd].name[0] != '\0'; cmd++)
    {
...
void do_wrlist(CHAR_DATA *ch, char *argument)
{
    ...
    if (((uvnum == 0) && (lvnum == 0)) || (arg[0] == '\0') || (type == -1))
    {
...
void do_newcorpse(CHAR_DATA *ch, char *argument)
{
    ...
    if (arg2[0] == '\0') /* show 'em */
    {
        ...
        if (checksum1 == corpse_cont[c] && checksum2 == item_level[c])
            ...
            long flag, long flag_skip, int min_level)
{
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        if (d->connected == CON_PLAYING && IS_IMMORTAL(d->character)
            && IS_SET(d->character->wiznet, WIZ_ON) && (!flag || IS_SET(d->character->wiznet, flag))
