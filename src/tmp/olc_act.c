...
bool redit_mlist(CHAR_DATA *ch, char *argument)
{
    ...
    for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++)
    {
        if ((pMobIndex = get_mob_index(vnum)))
        {
            if (fAll || is_name(ch, arg, pMobIndex->player_name) || (pMobIndex->level == atoi(arg)))
            {
                found = TRUE;
                sprintf(buf, "&z[&R%5d&z] &w%-17.16s",
                        pMobIndex->vnum,
...
bool redit_olist(CHAR_DATA *ch, char *argument)
{
    ...
    for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++)
    {
        if ((pObjIndex = get_obj_index(vnum)))
        {
            if (fAll || is_name(ch, arg, pObjIndex->name) || (flag_value(type_flags, arg) == pObjIndex->item_type) ||
...
bool aedit_show(CHAR_DATA *ch, char *argument)
{
    ...
    sprintf(buf, "&WSounds:   &w%s\n\r",
            pArea->reset_sound ? (str_cmp(pArea->reset_sound, "none") ? pArea->reset_sound : "(no sound)") : "(default: &GYou shiver as a cold breeze blows through the room.&w)");
    send_to_char(C_DEFAULT, buf, ch);
 ...
 */
        ...
        snprintf(buf, sizeof(buf), "New room vnum: %s", arg);
...
bool redit_rreset(CHAR_DATA *ch, char *argument)
{
    ...
    else
    {
        for (direc = 0; direc < 6; direc++)
            if (UPPER(arg[0]) == dir_name[direc][0])
                break;
    }
...
bool set_ac_setspell(CHAR_DATA *ch, char *argument)
{
    ...
    if (argument[0] == '\0' || spn == -1 || (*skill_table[spn].spell_fun) == (*spell_null))
    {
...
bool oedit_show(CHAR_DATA *ch, char *argument)
{
    ...
    send_to_char(C_DEFAULT, buf, ch);
    if ((pObj->ac_type == 5) && (pObj->ac_spell != NULL))
        sprintf(buf, "&cInvoke Spell&w:&z[&W%s&z]\n\r",
                pObj->ac_spell);
    else
        sprintf(buf, "&cInvoke Spell&w:&z[&W!NONE!&z]\n\r");
    send_to_char(C_DEFAULT, buf, ch);
    if (pObj->extra_descr)
    {
        EXTRA_DESCR_DATA *ed;
...
bool oedit_addaffect(CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(
            ...
            "none"))
    {
        snprintf(buf, sizeof(buf), "Unknown affect %s.\n\r", loc);
        send_to_char(AT_GREY, buf, ch);
        return FALSE;
    }
...
bool cedit_show(CHAR_DATA *ch, char *argument)
{
    ...
    sprintf(buf, "&PRecall:&Y      [%5d] %s\n\r", pClan->recall,
            get_room_index(pClan->recall)
                ? get_room_index(pClan->recall)->name
                : "none");
    send_to_char(C_DEFAULT, buf, ch);
...
bool cedit_induct(CHAR_DATA *ch, char *argument)
{
    ...
    {
        case 4:
            TOGGLE_BIT(pClan->settings, CLAN_CHAMP_INDUCT);
            break;
        case 3:
            TOGGLE_BIT(pClan->settings, CLAN_LEADER_INDUCT);
            break;
        case 2:
            TOGGLE_BIT(pClan->settings, CLAN_FIRST_INDUCT);
            break;
        case 1:
            TOGGLE_BIT(pClan->settings, CLAN_SECOND_INDUCT);
            break;
        default:
            break;
    }
...
bool forge_addaffect(CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_prefix(loc, "strength"))
    {
        ...
        {
            snprintf(buf, sizeof(buf), "You may not add more than %d to %s.\n\r",
                     max_stat, loc);
            send_to_char(AT_GREY, buf, ch);
            return FALSE;
        }
    ...
    if (!str_prefix(loc, "damroll"))
    {
        ...
        {
            snprintf(buf, sizeof(buf), "You may not add more than %d %s.\n\r",
                     max_dam, loc);
            send_to_char(AT_GREY, buf, ch);
            return FALSE;
        }
    ...
    if (!str_prefix(loc, "hitpoints") || !str_cmp(loc, "hp"))
    {
        ...
        {
            snprintf(buf, sizeof(buf), "You may not add more than %d %ss.\n\r",
                     max_hp, loc);
            send_to_char(AT_GREY, buf, ch);
            return FALSE;
        }
    ...
    if (!legal)
    {
        snprintf(buf, sizeof(buf), "Unknown affect %s, please choose from the list.\n\r", loc);
        send_to_char(AT_GREY, buf, ch);
        return FALSE;
    }
    cnt = 0;
    ...
    for (pAf = pObj->affected; pAf; pAf = pAf->next)
    {
        ...
        {
            snprintf(buf, sizeof(buf), "You have already added %d %s to this item.\n\r",
                     pAf->modifier, loc);
            send_to_char(AT_GREY, buf, ch);
            return FALSE;
        }
        if (pAf->location == APPLY_STR || pAf->location == APPLY_DEX || pAf->location == APPLY_INT
            || pAf->location == APPLY_WIS || pAf->location == APPLY_CON)
            stat_cnt++;
        if (stat_cnt >= max_statn
            && (!str_cmp(loc, "strength") || !str_cmp(loc, "dexterity")
                || !str_cmp(loc, "intelligence") || !str_cmp(loc, "wisdom")
    ...
    pAf->next = pObj->affected;
    pObj->affected = pAf;
    snprintf(buf, sizeof(buf), "Added %d %s for a cost of %d.\n\r", Mod, loc, cost);
    send_to_char(C_DEFAULT, buf, ch);
#ifdef NEW_MONEY
    pObj->cost.gold += cost;
#else
...
bool spedit_min(CHAR_DATA *ch, char *argument)
{
    ...
    {
        if ((!str_cmp(class, class_table[iClass].who_name) && !lng) || (!str_cmp(class, class_table[iClass].who_long) && lng))
        {
            EDIT_SPELL(ch, pSpell);
