...
#define CON_HOTREBOOT_RECOVER    250
...
#define PERCENT_LEFT(a, b) (100 - (((a - b) / (a + 0.00)) * 100))
#define PERCENT(a, b)      (((a - b) / (a + 0.00)) * 100)
...
#define MANA_COST(ch, sn)        (IS_NPC(ch) ? 0 : UMAX(skill_table[sn].min_mana, 100 / (2 + ch->level - skill_table[sn].skill_level[prime_class(ch)])))
#define SPELL_COST(ch, sn)       (is_class(ch, CLASS_VAMPIRE) ? MANA_COST(ch, sn) / 4 : MANA_COST(ch, sn))
#define MT(ch)                   (is_class(ch, CLASS_VAMPIRE) ? ch->bp : ch->mana)
#define MT_LOSE(ch, amt)         (is_class((ch), CLASS_VAMPIRE) ? ((ch)->bp -= (amt)) : ((ch)->mana -= (amt)))
#define MT_MAX(ch)               (is_class(ch, CLASS_VAMPIRE) ? MAX_BP(ch) : MAX_MANA(ch))
...
#define CAN_FLY(ch)              ((ch->race == RACE_PIXIE) || (ch->race == RACE_ELDER) || (IS_AFFECTED(ch, AFF_FLYING)))
/*
 * Object macros.
 */
...
void build_who_string args((CHAR_DATA * ch, char *buf3, char *argument));
...
void info             args((const char *str, long param1, long param2));
void challenge        args((const char *str, long param1, long param2));
...
void arena_chann      args((const char *str, long param1, long param2));
