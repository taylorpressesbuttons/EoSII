...
bool get_obj(CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container, bool palming)
{
    ...
    if (container)
    {
        ...
        obj_from_obj(obj);
        if (ch->in_room->vnum == ROOM_VNUM_DON_1 || ch->in_room->vnum == ROOM_VNUM_DON_2
            || ch->in_room->vnum == ROOM_VNUM_DON_3 || ch->in_room->vnum == ROOM_VNUM_DON_4
    ...
    if (arg2[0] == '\0')
    {
        ...
        else
        {
            ...
            for (obj = ch->in_room->contents; obj; obj = obj_next)
            {
                ...
                if ((arg1[3] == '\0' || is_name(ch, &arg1[4], obj->name)) && can_see_obj(ch, obj))
                {
                    found = TRUE;
...
void do_drop(CHAR_DATA *ch, char *argument)
{
    ...
    else
    {
        ...
        for (obj = ch->carrying; obj; obj = obj_next)
        {
            ...
            if ((arg[3] == '\0' || is_name(ch, &arg[4], obj->name)) && can_see_obj(ch, obj)
...
void do_eat(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_IMMORTAL(ch))
    {
        if (obj->item_type != ITEM_FOOD && obj->item_type != ITEM_PILL
            && obj->item_type != ITEM_BERRY && obj->item_type != ITEM_CORPSE_NPC)
        {
...
void wear_obj(CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace)
{
    ...
    {
        if (get_eq_char(ch, WEAR_ORBIT) && get_eq_char(ch, WEAR_ORBIT_2)
            && !remove_obj(ch, WEAR_ORBIT, fReplace) && !remove_obj(ch, WEAR_ORBIT_2, fReplace))
            return;
...
void do_dual(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_dual))
    {
    ...
    if (!str_cmp(arg, "all"))
    {
        ...
        for (obj = ch->carrying; obj; obj = obj_next)
        {
            ...
            if (obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj)
                && !(IS_AFFECTED2(ch, AFF_RAGE) && obj->item_type == ITEM_WEAPON))
...
void do_sacrifice(CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(arg, "all"))
    {
        ...
        for (obj = ch->in_room->contents; obj; obj = obj_next)
        {
            ...
            if (obj->deleted)
                continue;
            if (CAN_WEAR(obj, ITEM_TAKE) && obj->item_type != ITEM_MONEY)
    ...
    snprintf(buf, sizeof(buf), "%s gives you %s for your sacrifice.\n\r",
             msgbuf[iname], buf1);
...
void do_quaff(CHAR_DATA *ch, char *argument)
{
    ...
    if (obj->value[1] == skill_lookup("chaos field") || obj->value[2] == skill_lookup("chaos field")
    ...
    if (obj->value[1] == skill_lookup("vibrate") || obj->value[2] == skill_lookup("vibrate")
    ...
    if (obj->value[1] == skill_lookup("iceshield") || obj->value[2] == skill_lookup("iceshield")
...
void do_recite(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !(number_percent() < ch->pcdata->learned[gsn_scrolls]))
    {
...
void do_brandish(CHAR_DATA *ch, char *argument)
{
    ...
    if ((sn = staff->value[3]) < 0 || !is_sn(sn) || skill_table[sn].spell_fun == 0)
    {
...
void do_zap(CHAR_DATA *ch, char *argument)
{
    ...
    if ((wand->value[2] > 0) || (wand->value[1] == -1))
    {
        ...
        if (!IS_NPC(ch) && !(number_percent() < ch->pcdata->learned[gsn_wands]))
        {
            switch (number_bits(3))
            {
                case 0:
...
void do_buy(CHAR_DATA *ch, char *argument)
{
    ...
    else
    {
        ...
        if (!IS_NPC(ch) && ch->pcdata->learned[gsn_haggle] > 0 && number_percent() < 75)
        {
            cost->copper *= 0.85;
...
#else
    if ((cost = get_cost(keeper, obj, FALSE)) <= 0 || obj->level >= LEVEL_IMMORTAL)
    {
        act(AT_CYAN, "$n looks uninterested in $p.", keeper, obj, ch, TO_VICT);
        return;
    }
...
void do_poison_weapon(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_poison_weapon))
    {
    ...
    if (!IS_NPC(ch) && (get_curr_dex(ch) < 20 || ch->pcdata->condition[COND_DRUNK] > 0))
    {
    ...
    if (!IS_NPC(ch) && number_percent() > ch->pcdata->learned[gsn_poison_weapon])
    {
...
void do_invoke(CHAR_DATA *ch, char *argument)
{
    ...
    switch (obj->ac_type)
    {
        ...
        {
            spellarg[0] = '\0';
            snprintf(spellarg, sizeof(spellarg), "'%s' %s", obj->ac_spell, arg2);
...
void do_deposit(CHAR_DATA *ch, char *argument)
{
    ...
    if ((!is_number(arg) && (str_cmp(arg, "all"))) || (money > ch->gold) || (money < 0))
    {
...
void do_withdraw(CHAR_DATA *ch, char *argument)
{
    ...
    if ((!is_number(arg) && str_cmp(arg, "all")) || money > ch->pcdata->bankaccount || money < 0)
    {
        send_to_char(AT_WHITE, "Invalid ammount of money.\n\r", ch);
        return;
    }
...
void do_repair(CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(arg, "all"))
    {
        ...
        for (pObj = ch->carrying; pObj; pObj = pObj->next_content)
        {
            ...
            {
                case ITEM_WEAPON:
                    amount.gold = (amount.gold * pObj->value[2]) / (!pObj->value[1] ? 1 : pObj->value[1]);
                    amount.silver = (amount.silver * pObj->value[2]) / (!pObj->value[1] ? 1 : pObj->value[1]);
                    amount.copper = (amount.copper * pObj->value[2]) / (!pObj->value[1] ? 1 : pObj->value[1]);
                    break;
                case ITEM_ARMOR:
                    amount.gold = (amount.gold * pObj->level) / (!pObj->value[0] ? 1 : pObj->value[0]);
                    amount.silver = (amount.silver * pObj->level) / (!pObj->value[0] ? 1 : pObj->value[0]);
                    amount.copper = (amount.copper * pObj->level) / (!pObj->value[0] ? 1 : pObj->value[0]);
                    break;
                default:
    ...
    if (!str_cmp(arg, "all"))
    {
        ...
        for (pObj = ch->carrying; pObj; pObj = pObj->next_content)
        {
            ...
            {
                case ITEM_WEAPON:
                    cost = (cost * pObj->value[2]) / (!pObj->value[1] ? 1 : pObj->value[1]);
                    break;
                case ITEM_ARMOR:
                    cost = (cost * pObj->level) / (!pObj->value[0] ? 1 : pObj->value[0]);
                    break;
                default:
                    bug("Do_repair: Item not weapon or armor.", 0);
                    break;
...
void do_account(CHAR_DATA *ch, char *argument)
{
    ...
    if ((IS_SET(ch->in_room->area->area_flags, AREA_CLAN_HQ)) && (clanacct != 0))
    {
...
void do_alchemy(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_NPC(ch))
        return;
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_alchemy))
    {
    ...
    */
    ...
    if ((sn = skill_lookup(arg1)) < 0 || !can_use_skpell(ch, sn) || sn == skill_lookup("true sight"))
    {
    ...
    chance = ch->pcdata->learned[gsn_alchemy] - (skill_table[sn].skill_level[prime_class(ch)] / 6);
...
void do_scribe(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_NPC(ch))
        return;
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_scribe))
    {
...
void do_donate(CHAR_DATA *ch, char *argument)
{
    ...
    if (obj->item_type == ITEM_SCROLL || obj->item_type == ITEM_STAFF || obj->item_type == ITEM_WAND
...
void do_gravebind(CHAR_DATA *ch, char *argument)
{
    ...
    char arg[MAX_INPUT_LENGTH];
    int hp;
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_gravebind))
    {
...
{
    if (!can_use_skpell(ch, gsn_snatch) || ch->pcdata->learned[gsn_snatch] < number_percent())
    {
...
{
    OBJ_DATA *flask;
    if (IS_NPC(ch) || !can_use_skpell(ch, gsn_antidote))
    {
        send_to_char(C_DEFAULT, "Huh?\n\r", ch);
        return;
    }
