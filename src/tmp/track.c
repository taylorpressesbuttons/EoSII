...
void do_track(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_track))
    {
    ...
    switch (dir)
    {
        ...
        case TRACK_NO_PATH:
            sprintf(buf, "You can't sense a trail to %s from here.\n\r",
                    vict->sex == SEX_FEMALE ? "her" : vict->sex == SEX_MALE ? "him"
                                                                            : "it");
...
void hunt_victim(CHAR_DATA *ch)
{
    ...
    /*   for (found = FALSE, tmp = char_list; tmp && !found; tmp = tmp->next)
   {
    ...
    if (dir < 0)
    {
        if (IS_NPC(ch))
        {
            sprintf(buf, "Damn!  Lost %s!", ch->hunting->sex == SEX_FEMALE ? "her" : ch->hunting->sex == SEX_MALE ? "him"
                                                                                                                  : "it");
            do_say(ch, buf);
        }
        else if (ch->desc)
    ...
    else
    {
        if (IS_NPC(ch))
        {
            ...
            sprintf(log_buf, "%s hunting %s at %d.", ch->short_descr,
                    (IS_NPC(ch->hunting)) ? ch->hunting->short_descr : ch->hunting->name, ch->hunting->in_room->vnum);
