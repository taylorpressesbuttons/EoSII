...
void violence_update(void)
{
    ...
    for (ch = char_list; ch; ch = ch->next)
    {
        ...
        if (IS_AFFECTED(ch, AFF_BLIND) || (IS_NPC(ch) && ch->pIndexData->pShop))
            continue;
	 ...
	 */
        ...
        {
            if (rch->deleted || !IS_AWAKE(rch) || !(victim = rch->fighting))
                continue;

            if (!IS_NPC(ch) && (!IS_NPC(rch) || IS_AFFECTED(rch, AFF_CHARM)) && is_same_group(ch, rch) && IS_NPC(victim))
                break;
	 ...
	 */
        if (mobfighting)
        {
            ...
            {
                if (can_see(ch, vch) && (vch->level > 5) && is_same_group(vch, victim) && number_range(0, number) == 0)
                {
                    victim = vch;
                    number++;
                }
            ...
            if ((rch->pIndexData != ch->pIndexData && number_bits(3) != 0)
                || (IS_GOOD(ch) && IS_GOOD(victim)) || abs(victim->level - ch->level) > 3)
                continue;
        }
...
void one_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
    ...
    if (!IS_NPC(ch))
        thac0 = interpolate(ch->level, thac0_00, thac0_97) - GET_HITROLL(ch);
    else
        thac0 = interpolate(ch->level, thac0_00, thac0_97) - (ch->level + ch->level / 2);
...
*/
    ...
    else
    {
        ...
        else if ((is_class(ch, CLASS_WEREWOLF) || is_class(ch, CLASS_MONK)) && !wield)
    ...
    if (wield && IS_SET(wield->extra_flags, ITEM_POISONED))
        dam += dam / 8;
    if (
        (wield && IS_SET(wield->extra_flags, ITEM_FLAME)) || (!wield && is_affected(ch, gsn_flamehand)))
        dam += dam / 8;
    if (
        (wield && IS_SET(wield->extra_flags, ITEM_CHAOS)) || (!wield && is_affected(ch, gsn_chaoshand)))
        dam += dam / 4;
    if (
        (wield && IS_SET(wield->extra_flags, ITEM_ICY)) || (!wield && is_affected(ch, gsn_frosthand)))
        dam += dam / 8;
...
void one_dual(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
     ...
     * Guard against weird room-leavings.
     */
    if (!(get_eq_char(ch, WEAR_WIELD_2)) && !is_class(ch, CLASS_WEREWOLF) && !is_class(ch, CLASS_MONK))
        return;
    if (is_class(ch, CLASS_WEREWOLF) && (get_eq_char(ch, WEAR_SHIELD) || get_eq_char(ch, WEAR_HOLD)))
        return;
    if (is_class(ch, CLASS_MONK) && (get_eq_char(ch, WEAR_SHIELD) || get_eq_char(ch, WEAR_HOLD)))
        return;
...
void damage(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt)
{
    ...
    if (dam > 3500)
    {
        ...
        if (dt != 91 && ch->level <= LEVEL_HERO && dt != 40)
        {
...
*/
	 ...
	 */
        if (dt >= TYPE_HIT || dt == gsn_feed || dt == gsn_jab_punch || dt == gsn_kidney_punch
            || dt == gsn_cross_punch || dt == gsn_roundhouse_punch || dt == gsn_uppercut_punch
            || dt == gsn_punch || dt == gsn_flykick || dt == gsn_high_kick || dt == gsn_jump_kick
            || dt == gsn_spin_kick || dt == gsn_kick)
        {
            int leveldiff = ch->level - victim->level;

            if (IS_NPC(ch)
                && number_percent() < (leveldiff < -5 ? ch->level / 2 : UMAX(10, leveldiff))
                && dam == 0 && number_bits(4) == 0)
                disarm(ch, victim);
            if (IS_NPC(ch)
                && number_percent() < (leveldiff < -5 ? ch->level / 2 : UMAX(20, leveldiff))
                && dam == 0 && number_bits(4) == 0)
                trip(ch, victim);
        ...
        if (!IS_NPC(ch) && (dt == 1014 || dt == 1015 /* FIST & CLAW   */
                            || (dt == 1005           /* DAMAGE - Hann */
                                && is_class(ch, CLASS_WEREWOLF) && is_class(ch, CLASS_MONK) && is_bare_hand(ch))) &&
            ch->pcdata->learned[gsn_anatomyknow] > 0 && number_percent() <= ch->pcdata->learned[gsn_anatomyknow] / 9)
        {
        ...
        if (IS_AFFECTED2(victim, AFF_DANCING))
            dam += dam / 2;
        if (IS_AFFECTED(victim, AFF_FIRESHIELD) && !(dt == gsn_backstab && chance(number_range(5, 10))))
            dam -= dam / 8;
        if (IS_AFFECTED(victim, AFF_ICESHIELD) && !(dt == gsn_backstab && chance(number_range(5, 10))))
            dam -= dam / 8;
        if (IS_AFFECTED(victim, AFF_CHAOS) && !(dt == gsn_backstab && chance(number_range(5, 10))))
            dam -= dam / 4;
        if (IS_AFFECTED(victim, AFF_SHOCKSHIELD) && !(dt == gsn_backstab && chance(number_range(5, 10))))
            dam -= dam / 4;
        if (IS_AFFECTED(victim, AFF_VIBRATING) && !(dt == gsn_backstab && chance(number_range(5, 10))))
            dam -= dam / 4;
        if (IS_AFFECTED2(victim, AFF_INERTIAL))
            dam -= dam / 8;
        if (IS_SET(victim->act, UNDEAD_TYPE(victim)))
            dam -= dam / 8;
        if (IS_AFFECTED2(victim, AFF_BLADE) && !(dt == gsn_backstab && chance(number_range(5, 10))))
            dam -= dam / 4;
        if (IS_AFFECTED2(victim, AFF_FIELD) && !(dt == gsn_backstab && chance(number_range(5, 10))))
            dam -= dam / 4;
        if (IS_AFFECTED(victim, AFF_PROTECT) && IS_EVIL(ch))
            dam -= dam / 4;
        if (IS_AFFECTED2(victim, AFF_PROTECTION_GOOD) && IS_EVIL(ch))
            dam -= dam / 4;
        if (dam < 0)
            dam = 0;
    }
    ...
    if (dam > 0 && dt > TYPE_HIT
        && ((is_wielding_poisoned(ch) && !saves_spell(ch->level, victim))
    ...
    if (dam > 0 && dt > TYPE_HIT && is_wielding_flaming(ch) && number_percent() < 10)
    ...
    if (dam > 0
        && (dt == 1014 || dt == 1015
            || (dt == 1005 && is_class(ch, CLASS_WEREWOLF) && is_class(ch, CLASS_MONK)
                && is_bare_hand(ch)))
        && is_affected(ch, gsn_flamehand) && number_percent() < 33)
        spell_burning_hands(skill_lookup("burning hands"),
                            ch->level, ch, victim);
    if (victim->position == POS_DEAD || ch->in_room != victim->in_room)
        return;

    if (dam > 0 && dt > TYPE_HIT && is_wielding_icy(ch) && number_percent() < 20)
    ...
    if (dam > 0
        && (dt == 1014 || dt == 1015
            || (dt == 1005 && is_class(ch, CLASS_WEREWOLF) && is_class(ch, CLASS_MONK)
                && is_bare_hand(ch)))
        && is_affected(ch, gsn_frosthand) && number_percent() < 20)
        spell_chill_touch(skill_lookup("chill touch"), ch->level * 3,
                          ch, victim);
    if (victim->position == POS_DEAD || ch->in_room != victim->in_room)
        return;

    if (dam > 0 && dt > TYPE_HIT && is_wielding_chaos(ch) && number_percent() < 20)
    ...
    if (dam > 0
        && (dt == 1014 || dt == 1015
            || (dt == 1005 && is_class(ch, CLASS_WEREWOLF) && is_class(ch, CLASS_MONK)
                && is_bare_hand(ch)))
        && is_affected(ch, gsn_chaoshand) && number_percent() < 10)
        spell_energy_drain(skill_lookup("energy drain"),
                           ch->level * 1.5, ch, victim);
    if (victim->position == POS_DEAD || ch->in_room != victim->in_room)
        return;

    if (IS_SET(ch->act, UNDEAD_TYPE(ch)) && !saves_spell(ch->level, victim)
        && !is_class(victim, CLASS_NECROMANCER) && victim->race != RACE_GHOUL)
    {
        AFFECT_DATA af;
    ...
    if (victim->position == POS_DEAD)
    {
        if (!IS_ARENA(ch))
        {
            ...
            if (((ch->guild != NULL) ? ch->guild->type & GUILD_CHAOS : 0)
                && ch->guild == victim->guild && victim->guild_rank > ch->guild_rank)
            {
                int temp;
                temp = ch->guild_rank;
	     ...
	     */
                if (victim->level < LEVEL_HERO || (victim->level >= LEVEL_HERO && IS_NPC(ch)))
                ...
                if (!IS_NPC(ch) && IS_SET(victim->act, PLR_THIEF) && ch->guild && !strcmp(ch->guild->name, "MERCENARY"))
                {
                    REMOVE_BIT(victim->act, PLR_THIEF);
                    info("%s the puny thief gets destroyed by the &rMERCENARY&C %s!",
                         (long)victim->name, (long)ch->name);
                }
                else if (!IS_NPC(ch) && IS_SET(ch->act, PLR_THIEF) && victim->guild && !strcmp(victim->guild->name, "MERCENARY"))
                    info("%s, the sly thief, has killed the &rMERCENARY&C %s.",
                         (long)ch->name, (long)victim->name);
                else
                    info("%s gets slaughtered by %s!", (long)victim->name,
                         (long)(IS_NPC(ch) ? ch->short_descr : ch->name));
                save_clans();
            }
        }
        else
...
*/
                ...
                {
                    obj_lose->cost.gold = (obj_lose->cost.gold > 0) ? (obj_lose->cost.gold - dam / 6) : 0;
                    obj_lose->cost.silver = (obj_lose->cost.silver > 0) ? (obj_lose->cost.silver - dam / 6) : 0;
                    obj_lose->cost.copper = (obj_lose->cost.copper > 0) ? (obj_lose->cost.copper - dam / 6) : 0;
                }
                /*		 if ( (obj_lose->cost.gold +
 		      (obj_lose->cost.silver/SILVER_PER_GOLD) +
		      (obj_lose->cost.copper/COPPER_PER_GOLD) ) < 0 )
...
bool is_safe(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    if (IS_SET(victim->act, PLR_THIEF) && (ch->guild && !strcmp(ch->guild->name, "MERCENARY")))
        return FALSE;
    if (IS_SET(ch->act, PLR_THIEF) && (victim->guild && !strcmp(victim->guild->name, "MERCENARY")))
        /*	return FALSE; */
        return TRUE;
...
void check_killer(CHAR_DATA *ch, CHAR_DATA *victim)
{
     ...
     */
    if (IS_NPC(ch) || ch == victim || IS_SET(ch->act, PLR_KILLER) || IS_SET(ch->act, PLR_THIEF))
        return;
    pClan = get_clan_index(ch->clan);
    if (/*ch->clan != 0 ||*/ (IS_SET(pClan->settings, CLAN_PKILL)) ||
...
bool is_wielding_flaming(CHAR_DATA *ch)
{
    ...
    if ((obj = get_eq_char(ch, WEAR_WIELD)) && IS_SET(obj->extra_flags, ITEM_FLAME))
        return TRUE;
...
bool is_wielding_icy(CHAR_DATA *ch)
{
    ...
    if ((obj = get_eq_char(ch, WEAR_WIELD)) && IS_SET(obj->extra_flags, ITEM_ICY))
        return TRUE;
...
bool is_wielding_chaos(CHAR_DATA *ch)
{
    ...
    if ((obj = get_eq_char(ch, WEAR_WIELD)) && IS_SET(obj->extra_flags, ITEM_CHAOS))
        return TRUE;
...
bool check_dodge(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    if (IS_NPC(victim))
        ...
        chance = victim->pcdata->learned[gsn_dodge] / 2 + victim->pcdata->learned[gsn_dodge_two] / 2;
...
void make_corpse(CHAR_DATA *ch)
{
    ...
    if (IS_ARENA(ch))
    {
        ...
        sprintf(log_buf, "&C%s &chas been awarded &W%d &ccoins for %s victory.",
                gch->name, award, (gch->sex == SEX_NEUTRAL ? "its" : (gch->sex == SEX_MALE ? "his" : "her")));
        /* wiznet(log_buf, NULL, NULL, WIZ_DEATHS, 0, 0); */
        log_string(log_buf, CHANNEL_LOG, -1);
        challenge(log_buf, 0, 0);
#ifdef NEW_MONEY
...
int xp_compute(CHAR_DATA *gch, CHAR_DATA *victim)
{
    ...
    if (victim->level + 5 <= gch->level)
        return 0;
    /* 3-4 levels lower */
    if (victim->level + 3 == gch->level || victim->level + 4 == gch->level)
    {
        xp = (gch->level < 10) ? number_range(50, 100) + 10 : 0;
        return xp;
    }
    /* if same lvl or up to 2 lvls lower */
    if (victim->level > gch->level - 3 && victim->level <= gch->level)
    {
...
void dam_message(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt)
{
    ...
    if (dt == TYPE_HIT)
    {
        if (dam > 0)
        {
            sprintf(buf1, "You &r%s&X $N for %d%c", vs, dam, punct);
            sprintf(buf2, "$n &G%s&X you for %d%c", vp, dam, punct);
            sprintf(buf3, "$n &z%s&X $N for %d%c", vp, dam, punct);
            sprintf(buf4, "You &G%s&X yourself for %d%c", vs, dam, punct);
            sprintf(buf5, "$n &z%s&X $mself for %d%c", vp, dam, punct);
        }
        else
        {
    ...
    else
    {
        ...
        if (dt > TYPE_HIT && is_wielding_poisoned(ch))
        {
            if (dam > 0)
            {
                sprintf(buf1, "Your &gpoisoned&X %s &r%s&X $N for %d%c", attack, vp, dam, punct);
                sprintf(buf2, "$n's &gpoisoned&X %s &G%s&X you for %d%c", attack, vp, dam, punct);
                sprintf(buf3, "$n's &gpoisoned&X %s &z%s&X $N for %d%c", attack, vp, dam, punct);
                sprintf(buf4, "Your &gpoisoned&X %s &G%s&X you for %d%c", attack, vp, dam, punct);
                sprintf(buf5, "$n's &gpoisoned&X %s &z%s&X $n for %d%c", attack, vp, dam, punct);
            }
            else
            {
        ...
        else
        {
            if (dam > 0)
            {
                sprintf(buf1, "Your %s &r%s&X $N for %d%c", attack, vp, dam, punct);
                sprintf(buf2, "$n's %s &G%s&X you for %d%c", attack, vp, dam, punct);
                sprintf(buf3, "$n's %s &z%s&X $N for %d%c", attack, vp, dam, punct);
                sprintf(buf4, "Your %s &G%s&X you for %d%c", attack, vp, dam, punct);
                sprintf(buf5, "$n's %s &z%s&X $n for %d%c", attack, vp, dam, punct);
            }
            else
            {
...
{
    if ((IS_AFFECTED(victim, AFF_FLYING)) || (victim->race == RACE_PIXIE) || (victim->race == RACE_ELDER))
        return;
...
void do_murder(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_SET(victim->act, PLR_PKILLER) && !IS_ARENA(victim) && !IS_NPC(victim)) /* chars can murder mobs */
    {
...
void do_backstab(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_backstab))
    {
        send_to_char(C_DEFAULT,
...
void do_rescue(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_rescue))
    {
        send_to_char(C_DEFAULT,
...
void do_gouge(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_gouge))
    {
        send_to_char(C_DEFAULT,
...
void do_high_kick(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_high_kick))
    {
        send_to_char(C_DEFAULT,
                     "You'd better leave the martial arts to warriors.\n\r", ch);
        return;
    ...
    if (!victim || victim->position == POS_DEAD || !victim->in_room || victim->in_room != ch->in_room)
        return;
...
void do_jump_kick(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_jump_kick))
    {
        send_to_char(C_DEFAULT,
                     "You'd better leave the martial arts to warriors.\n\r", ch);
        return;
...
void do_spin_kick(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_spin_kick))
    {
        send_to_char(C_DEFAULT,
                     "You'd better leave the martial arts to warriors.\n\r", ch);
        return;
...
void do_punch(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_punch))
    {
        send_to_char(C_DEFAULT,
...
void do_jab_punch(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_jab_punch))
    {
        send_to_char(C_DEFAULT,
                     "You'd better leave the martial arts to fighters.\n\r", ch);
        return;
...
void do_kidney_punch(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_kidney_punch))
    {
        send_to_char(C_DEFAULT,
                     "You'd better leave the martial arts to fighters.\n\r", ch);
        return;
...
void do_cross_punch(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_cross_punch))
    {
        send_to_char(C_DEFAULT,
                     "You'd better leave the martial arts to fighters.\n\r", ch);
        return;
...
void do_roundhouse_punch(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_roundhouse_punch))
    {
        send_to_char(C_DEFAULT,
                     "You'd better leave the martial arts to fighters.\n\r", ch);
        return;
...
void do_uppercut_punch(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_uppercut_punch))
    {
        send_to_char(C_DEFAULT,
                     "You'd better leave the martial arts to fighters.\n\r", ch);
        return;
...
void do_flury(CHAR_DATA *ch, char *argument)
{
    ...
    if (!victim || victim->position == POS_DEAD || !victim->in_room || victim->in_room != ch->in_room || count > 2)
        return;
...
void do_feed(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_feed))
    {
        send_to_char(AT_RED,
...
void do_disarm(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_disarm))
    {
        send_to_char(C_DEFAULT, "You don't know how to disarm opponents.\n\r", ch);
        return;
    }

    if ((!get_eq_char(ch, WEAR_WIELD)) && (!get_eq_char(ch, WEAR_WIELD_2)) && !is_class(ch, CLASS_WEREWOLF))
    {
...
void do_drain_life(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_drain_life))
    {
        send_to_char(C_DEFAULT, "You failed.\n\r", ch);
        return;
    }
...
void do_mental_drain(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_mental_drain))
    {
        send_to_char(C_DEFAULT, "You failed.\n\r", ch);
        return;
    }
...
void do_multiburst(CHAR_DATA *ch, char *argument)
{
    ...
    if (sn1 != -1 && can_use_skpell(ch, sn1) && skill_table[sn1].target != TAR_CHAR_SELF
        && (*skill_table[sn1].spell_fun) != (*spell_null))
        legal1 = TRUE;
    if (sn2 != -1 && can_use_skpell(ch, sn2) && skill_table[sn2].target != TAR_CHAR_SELF
...
void do_challenge(CHAR_DATA *ch, char *argument)
{
    ...
    if (rvnum == ROOM_VNUM_HELL || rvnum == ROOM_VNUM_RJAIL)
    {
    ...
    else if ((award = atoi(arg2)))
    {
        ...
        {
            if (d->connected == CON_PLAYING && is_name(NULL, arg1, d->character->name) && d->character && ch)
            {
                found = TRUE;
...
void do_bite(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_bite))
    {
...
void do_howl_of_fear(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && is_class(ch, gsn_howlfear))
    {
        send_to_char(C_DEFAULT, "You failed.\n\r", ch);
        return;
    }
...
void do_rake(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_rake))
    {
        send_to_char(C_DEFAULT,
...
void do_bladepalm(CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->pcdata->learned[gsn_bladepalm] > number_percent())
    {
        ...
        if (anatomy > 0 && number_percent() < number_range(anatomy / 9, anatomy / 4.5))
        {
            update_skpell(ch, gsn_anatomyknow);
            send_to_char(AT_RED, "You hit a pressure point!\n\r", ch);
            act(AT_RED, "$n hit one of $N's pressure points!",
...
void do_flyingkick(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_flykick))
    {
...
void do_nerve(CHAR_DATA *ch, char *argument)
{
    ...
    if (number_percent() < ch->pcdata->learned[gsn_nerve] && number_percent() < (ch->level * 75) / victim->level)
    {
        AFFECT_DATA af;
...
void do_shriek(CHAR_DATA *ch, char *argument)
{
    ...
    int blown = 0;
    int max_blown = 0;
    if (!IS_NPC(ch) && !can_use_skpell(ch, gsn_shriek))
    {
        send_to_char(AT_GREY, "Huh?\n\r", ch);
        return;
    }
