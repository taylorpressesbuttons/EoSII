...
void update_skpell(CHAR_DATA *ch, int sn)
{
    ...
    if (ch->pcdata->learned[sn] <= 0 || ch->pcdata->learned[sn] >= adept)
        return;
...
void say_spell(CHAR_DATA *ch, int sn)
{
    ...
    snprintf(buf2, sizeof(buf2), "$n %s the words, '%s'.",
             prime_class(ch) != CLASS_BARD ? "utters" : "sings", buf);
    sprintf(buf, "$n %s the words, '%s'.",
            prime_class(ch) != CLASS_BARD ? "utters" : "sings", skill_table[sn].name);
...
void do_cast(CHAR_DATA *ch, char *argument)
{
    ...
    if (!IS_NPC(ch))
    {
        ...
        else if ((ch->bp < mana) && (is_class(ch, CLASS_VAMPIRE)))
        {
...
void spell_astral(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (!(victim = get_char_world(ch, target_name))
        ...
        || IS_SET(victim->act, ACT_NOASTRAL) || IS_ARENA(ch) || victim->in_room->area == arena.area
...
void spell_pass_plant(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (!(victim = get_char_world(ch, target_name)) || victim->in_room->area != ch->in_room->area
        || IS_SET(victim->in_room->room_flags, ROOM_PRIVATE)
        || IS_SET(victim->in_room->room_flags, ROOM_SOLITARY)
        || victim->in_room->sector_type == SECT_INSIDE || victim->in_room->sector_type == SECT_CITY
        || victim->in_room->sector_type == SECT_AIR || IS_AFFECTED(victim, AFF_NOASTRAL))
    {
...
void spell_charm_person(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (!IS_NPC(victim))
        return;
    if (IS_AFFECTED(victim, AFF_CHARM) || IS_AFFECTED(ch, AFF_CHARM) || level < victim->level || saves_spell(level, victim))
        return;
...
void spell_enchant_weapon(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (obj->item_type != ITEM_WEAPON || IS_OBJ_STAT(obj, ITEM_MAGIC) || (obj->affected && !IS_OBJ_STAT(obj, ITEM_DWARVEN)))
    {
...
void spell_flame_blade(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (obj->item_type != ITEM_WEAPON || IS_OBJ_STAT(obj, ITEM_MAGIC)
        || IS_OBJ_STAT(obj, ITEM_FLAME) || (obj->affected && !IS_OBJ_STAT(obj, ITEM_DWARVEN)))
    {
...
void spell_chaos_blade(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (obj->item_type != ITEM_WEAPON || IS_OBJ_STAT(obj, ITEM_MAGIC)
        || IS_OBJ_STAT(obj, ITEM_CHAOS) || (obj->affected && !IS_OBJ_STAT(obj, ITEM_DWARVEN)))
    {
...
void spell_golden_aura(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (IS_AFFECTED2(victim, AFF_GOLDEN))
        return;
    if (!IS_NPC(ch) && !can_use_skpell(ch, sn))
    {
        send_to_char(AT_BLUE, "Nothing happens.\n\r", ch);
        return;
    }
...
void spell_permenancy(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (obj->item_type != ITEM_WAND && obj->item_type != ITEM_STAFF && obj->item_type != ITEM_LENSE)
    {
...
void spell_polymorph(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (!(victim = get_char_world(ch, target_name)) || victim == ch || saves_spell(level, victim)
...
void spell_portal(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (!(victim = get_char_world(ch, target_name)) || victim == ch || !victim->in_room
        ...
        || IS_SET(victim->in_room->room_flags, ROOM_SOLITARY)
        || IS_SET(victim->in_room->room_flags, ROOM_NO_ASTRAL_IN)
        || IS_SET(ch->in_room->room_flags, ROOM_NO_ASTRAL_OUT) || IS_ARENA(ch)
...
void spell_mind_probe(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    sprintf(buf, "%s%d/%d %s",
            is_class(victim, CLASS_VAMPIRE) ? "&R" : "&C", MT(ch), MT_MAX(ch),
...
*/
    ...
    sprintf(buf, "You are carrying %d/%d items with weight %d/%d kg.\n\r",
            victim->carry_number, can_carry_n(victim), victim->carry_weight, can_carry_w(victim));
    send_to_char(AT_CYAN, buf, ch);

    sprintf(buf, "Str: %d&p(&P%d&p)&P  Int: %d&p(&P%d&p)&P  Wis: %d&p(&P%d&p)&P  Dex: %d&p(&P%d&p)&P  Con: %d&p(&P%d&p)&P.\n\r",
    ...
    if (!IS_NPC(victim) && victim->pcdata->condition[COND_THIRST] == 0 && victim->level >= LEVEL_IMMORTAL)
        send_to_char(AT_BLUE, "You are thirsty.\n\r", ch);
    if (!IS_NPC(victim) && victim->pcdata->condition[COND_FULL] == 0 && victim->level >= LEVEL_IMMORTAL)
    ...
    if (GET_AC(victim) >= 101)
        send_to_char(AT_GREEN, "WORSE than naked!\n\r", ch);
    else if (GET_AC(victim) >= 20)
        send_to_char(AT_GREEN, "naked.\n\r", ch);
    else if (GET_AC(victim) >= 0)
        send_to_char(AT_GREEN, "wearing clothes.\n\r", ch);
    else if (GET_AC(victim) >= -50)
        send_to_char(AT_GREEN, "slightly armored.\n\r", ch);
    else if (GET_AC(victim) >= -100)
        send_to_char(AT_GREEN, "somewhat armored.\n\r", ch);
    else if (GET_AC(victim) >= -250)
        send_to_char(AT_GREEN, "armored.\n\r", ch);
    else if (GET_AC(victim) >= -500)
        send_to_char(AT_GREEN, "well armored.\n\r", ch);
    else if (GET_AC(victim) >= -750)
        send_to_char(AT_GREEN, "strongly armored.\n\r", ch);
    else if (GET_AC(victim) >= -1000)
        send_to_char(AT_GREEN, "heavily armored.\n\r", ch);
    else if (GET_AC(victim) >= -1200)
        send_to_char(AT_GREEN, "superbly armored.\n\r", ch);
    else if (GET_AC(victim) >= -1400)
        send_to_char(AT_GREEN, "divinely armored.\n\r", ch);
    else
    ...
    {
        sprintf(buf, "WizInvis level: %d   WizInvis is %s\n\r", victim->wizinvis, IS_SET(victim->act, PLR_WIZINVIS) ? "ON" : "OFF");
        send_to_char(AT_YELLOW, buf, ch);
        sprintf(buf, "Cloaked level: %d   Cloaked is %s\n\r", victim->cloaked, IS_SET(victim->act, PLR_CLOAKED) ? "ON" : "OFF");
        send_to_char(AT_YELLOW, buf, ch);
    }
    if (victim->affected)
    {
...
void spell_summon(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (!(victim = get_char_world(ch, target_name)) || victim == ch
        || ch->in_room->area == arena.area || IS_ARENA(victim) || !victim->in_room
        || IS_SET(victim->in_room->room_flags, ROOM_SAFE)
        || IS_SET(victim->in_room->room_flags, ROOM_PRIVATE)
        || IS_SET(victim->in_room->room_flags, ROOM_SOLITARY)
        || IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL)
        || IS_SET(victim->in_room->room_flags, ROOM_NO_ASTRAL_OUT) || victim->level >= level + 3
        || victim->fighting || (IS_NPC(victim) && saves_spell(level, victim))
        || IS_SET(victim->in_room->area->area_flags, AREA_PROTOTYPE)
        || IS_AFFECTED(victim, AFF_NOASTRAL))
    {
        send_to_char(AT_BLUE, "You failed.\n\r", ch);
    ...
    if (number_percent() < 2 * level && !saves_spell(level, victim))
    {
        for (obj_lose = victim->carrying; obj_lose; obj_lose = obj_next)
        {
		    ...
		    obj_lose->value[0] -= 1;*/
                        /* 5% -- Altrag */
#ifdef NEW_MONEY
                        obj_lose->cost.gold = (obj_lose->cost.gold > 0) ? (obj_lose->cost.gold - (obj_lose->cost.gold / 20)) : 0;
                        obj_lose->cost.silver = (obj_lose->cost.silver > 0) ? (obj_lose->cost.silver - (obj_lose->cost.silver / 20)) : 0;
                        obj_lose->cost.copper = (obj_lose->cost.copper > 0) ? (obj_lose->cost.copper - (obj_lose->cost.copper / 20)) : 0;
#else
...
void spell_disrupt(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (!is_affected(vch, sn) && (vch != victim || !saves_spell(level, victim)) &&
        number_bits(8) == 0)
    {
...
void spell_enhance_armor(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (number_percent() < ch->pcdata->learned[sn] / 2 + 3 * (ch->level - obj->level))
...
void check_dispel_aff(CHAR_DATA *victim, bool *found, int level, const char *spell, long vector)
{
    ...
    if (IS_AFFECTED(victim, vector) && !saves_spell(level, victim)
...
void check_dispel_aff2(CHAR_DATA *victim, bool *found, int level, const char *spell, long vector)
{
    int sn;
    sn = skill_lookup(spell);
    if (IS_AFFECTED2(victim, vector) && !saves_dispel(level, victim->level, -1) && !is_affected(victim, sn))
    {
        *found = TRUE;
...
void spell_dispel_magic(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    for (paf = victim->affected; paf; paf = paf->next)
    {
        if (paf->deleted)
            continue;
        if (skill_table[paf->type].spell_fun != spell_null && skill_table[paf->type].dispelable == TRUE)
...
void spell_cancellation(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if ((!IS_NPC(ch) && IS_NPC(victim) && !(IS_AFFECTED(ch, AFF_CHARM) && ch->master == victim))
        || (IS_NPC(ch) && !IS_NPC(victim)) || !is_same_group(ch, victim))
    {
    ...
    for (paf = victim->affected; paf; paf = paf->next)
    {
        if (skill_table[paf->type].spell_fun != spell_null && skill_table[paf->type].spell_fun != spell_poison)
            if (check_dispel(level, victim, paf->type))
                found = TRUE;
    }
...
void spell_turn_undead(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    chance = (level * (10 + (IS_GOOD(ch) ? 15 : IS_EVIL(ch) ? 0
                                                            : 10)));
    chance /= victim->level;
...
void spell_hex(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (IS_AFFECTED(victim, AFF_BLIND + AFF_CURSE) && IS_AFFECTED2(victim, AFF_CONFUSED))
    {
...
void spell_thunder_strike(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (IS_AFFECTED(victim, AFF_BLIND) && IS_AFFECTED2(victim, AFF_CONFUSED))
    {
        return;
    }
...
void spell_firewall(int sn, int level, CHAR_DATA *ch, void *vo)
{
    ...
    if (IS_SET(ch->in_room->room_flags, ROOM_SAFE) || IS_SET(ch->in_room->room_flags, ROOM_NO_OFFENSIVE))
